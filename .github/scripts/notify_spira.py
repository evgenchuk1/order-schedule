"""
Local (Task Scheduler, every 5 min) — counting schedule reminders for spira.html.
Sends consolidated daily count list (done/pending) to every manager via
Telegram (personal) + Email at 06:30, then resent hourly through 12:30 (last one).
"""
import json, sys
...
LOG_FILE = Path(__file__).resolve().parent.parent.parent / 'notify_spira.log'
...
STATE_FILE = Path(__file__).resolve().parent.parent.parent / 'notify_spira_state.json'
...
# ─── Static schedule data (must match spira.html CHOR/PACHAT/YOMI/SPECIAL_DATES) ─
CHOR = [
...
PACHAT = [
    ...
    {'d': '2026-08-24', 'items': ['סכו"ם'],               'codes': ['831']},
    {'d': '2026-08-31', 'items': ['מאגדות גלידה'],        'codes': ['203']},
    {'d': '2026-09-07', 'items': ['גלידות משפחתיות'],     'codes': ['202']},
    {'d': '2026-09-14', 'items': ['סטים מצעים'],          'codes': ['913']},
    {'d': '2026-09-21', 'items': ['עיתונים ומגזינים'],    'codes': ['955']},
    {'d': '2026-09-28', 'items': ['איפור'],                'codes': ['413','414']},
    {'d': '2026-10-05', 'items': ['קרמים'],               'codes': ['427']},
    {'d': '2026-10-12', 'items': ['בישום'],               'codes': ['400','401','402','403']},
    {'d': '2026-10-19', 'items': ['אביזרי סלולר'],        'codes': ['430']},
    {'d': '2026-10-26', 'items': ['דגים קפואים'],         'codes': ['112']},
    {'d': '2026-11-02', 'items': ['מוצרי קו קופות'],      'codes': ['690']},
    {'d': '2026-11-09', 'items': ['דג סלמון'],            'codes': ['114']},
    {'d': '2026-11-16', 'items': ['בשר קפוא חלקים'],      'codes': ['181']},
    {'d': '2026-11-23', 'items': ['אביזרי תינוקות'],      'codes': ['452']},
    {'d': '2026-11-30', 'items': ['אביזרי גילוח'],        'codes': ['438']},
]
...
def in_window(cur, target, window=7):
...
SEND_TIMES = [(6, 30), (7, 30), (8, 30), (9, 30), (10, 30), (11, 30), (12, 30)]

due_slot = None
for h, m in SEND_TIMES:
    target = h * 60 + m
    if in_window(cur_min, target):
        due_slot = f'{h:02d}{m:02d}'
        break
...
key = f'spira|{due_slot}|{today_str}'
if key in today_fired:
    print(f'Already sent slot {due_slot} today. Exiting.')
    sys.exit(0)

done_state = common.get_daily_state(BRANCH, today_str)

date_he = f"{now_il.day} {now_il.strftime('%B')} {now_il.year}"
tg_lines = [f'<b>📋 ריכוז ספירות — {date_he} ({now_il.strftime("%H:%M")})</b>', '']
...
print(f'--- Sending spira slot {due_slot} ---')
